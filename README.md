...
**贡献与积压按成员统计，滞留条目逐级提醒。** 「贡献与积压」看板统计每个成员的贡献量（数据来自审计日志 —— "回答追问"这类不落条目表的动作也不会漏记）与积压条目数。积压和滞留都按统一的负责人口径归集：被指派人 → 食谱创建者 → 空间所有者，没人认领的条目总有人兜底。超过 N 天没人碰的条目会在看板里标出负责人，提醒逐级升级：1 级只通知负责人，2 级叫上整理者，3 级全家知悉，到顶为止。级别记在审计日志里而不是条目表上 —— 提醒不算"处理"，不能碰条目的 `updatedAt`，否则催一次条目反而"变新鲜"、滞留天数永远涨不上去；条目一旦被真正处理，级别自动归零、重新从 1 级计。
...
**协作统计与逐级提醒**（`apps/server/tests/reminder.test.ts`，9 个断言组）：
按成员的贡献量与积压口径、滞留条目负责人归属（被指派人 → 食谱创建者 → 空间所有者）、逐级提醒的通知范围（负责人 → 整理者 → 全家）与到顶拒绝、条目被处理后级别归零重计、提醒本身不改变滞留时钟、以及旁观者不能提醒 / 非成员看不到统计的权限边界。

**UI 层闭环**（`apps/web/e2e/closed-loop.spec.ts`）：在真实 Chrome 里从注册走到发布，包含在波形上拖拽框选片段，最后验证导出真的能下载。

```bash
npm run test        # 后端 69 个测试
